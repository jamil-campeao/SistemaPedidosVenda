object frmInicial: TfrmInicial
  Left = 0
  Top = 0
  Caption = 'frmInicial'
  ClientHeight = 600
  ClientWidth = 1044
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -12
  Font.Name = 'Segoe UI'
  Font.Style = []
  KeyPreview = True
  WindowState = wsMaximized
  OnCreate = FormCreate
  OnKeyDown = FormKeyDown
  OnKeyPress = FormKeyPress
  OnShow = FormShow
  TextHeight = 15
  object Panel2: TPanel
    Left = 0
    Top = 559
    Width = 1044
    Height = 41
    Align = alBottom
    TabOrder = 0
    ExplicitTop = 558
    ExplicitWidth = 1040
    object btIncluirPedido: TBitBtn
      Left = 12
      Top = 6
      Width = 97
      Height = 25
      Caption = 'F3 - &Incluir'
      Glyph.Data = {
        
        00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
      TabOrder = 0
      OnClick = btIncluirPedidoClick
    end
    object btGravar: TBitBtn
      Left = 125
      Top = 6
      Width = 97
      Height = 25
      Caption = 'F7 - &Gravar'
      Glyph.Data = {
        
        00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
      TabOrder = 1
      OnClick = btGravarClick
    end
    object btSair: TBitBtn
      Left = 240
      Top = 6
      Width = 97
      Height = 25
      Caption = 'ESC - &Sair'
      Glyph.Data = {
        
        4E6E244E6E244E6E244E6E244E6E244E6E244E6E244E6EFF00FF}
      TabOrder = 2
      OnClick = btSairClick
    end
  end
  object tbControlGeral: TTabControl
    Left = 0
    Top = 0
    Width = 1044
    Height = 559
    Align = alClient
    Enabled = False
    TabOrder = 1
    ExplicitWidth = 1040
    ExplicitHeight = 558
    object pnlCliente: TPanel
      Left = 4
      Top = 6
      Width = 1036
      Height = 99
      Align = alTop
      TabOrder = 0
      ExplicitWidth = 1032
      object Label1: TLabel
        Left = 24
        Top = 26
        Width = 62
        Height = 15
        Caption = 'C'#243'd Cliente'
      end
      object Label2: TLabel
        Left = 176
        Top = 27
        Width = 33
        Height = 15
        Caption = 'Nome'
      end
      object lblCidade: TLabel
        Left = 450
        Top = 26
        Width = 37
        Height = 15
        Caption = 'Cidade'
      end
      object Label3: TLabel
        Left = 648
        Top = 26
        Width = 14
        Height = 15
        Caption = 'UF'
      end
      object Label4: TLabel
        Left = 24
        Top = 8
        Width = 42
        Height = 15
        Caption = 'Clientes'
      end
      object Label10: TLabel
        Left = 704
        Top = 26
        Width = 70
        Height = 15
        Caption = 'Data Emiss'#227'o'
      end
      object Label12: TLabel
        Left = 798
        Top = 26
        Width = 54
        Height = 15
        Caption = 'N'#186' Pedido'
      end
      object edCodCliente: TEdit
        Left = 24
        Top = 48
        Width = 48
        Height = 23
        Hint = 'Digite o C'#243'digo do Cliente ou F2 para Consultar'
        MaxLength = 10
        NumbersOnly = True
        ParentShowHint = False
        ShowHint = True
        TabOrder = 0
        OnExit = edCodClienteExit
        OnKeyDown = edCodClienteKeyDown
      end
      object edNomeCliente: TDBEdit
        Left = 176
        Top = 48
        Width = 251
        Height = 23
        TabStop = False
        DataField = 'CLIENTE_NOME'
        DataSource = dsCliente
        ReadOnly = True
        TabOrder = 4
      end
      object edCidade: TDBEdit
        Left = 450
        Top = 48
        Width = 172
        Height = 23
        TabStop = False
        DataField = 'CLIENTE_CIDADE'
        DataSource = dsCliente
        ReadOnly = True
        TabOrder = 5
      end
      object edUF: TDBEdit
        Left = 643
        Top = 48
        Width = 34
        Height = 23
        TabStop = False
        DataField = 'CLIENTE_UF'
        DataSource = dsCliente
        ReadOnly = True
        TabOrder = 6
      end
      object DBEdit1: TDBEdit
        Left = 704
        Top = 47
        Width = 70
        Height = 23
        TabStop = False
        DataField = 'PEDIDO_DATAEMISSAO'
        DataSource = dsPedidoCabecalho
        ReadOnly = True
        TabOrder = 7
      end
      object edNumeroPedido: TDBEdit
        Left = 798
        Top = 47
        Width = 54
        Height = 23
        TabStop = False
        BiDiMode = bdRightToLeft
        DataField = 'PEDIDO_NUMERO'
        DataSource = dsPedidoCabecalho
        Enabled = False
        Font.Charset = DEFAULT_CHARSET
        Font.Color = clWindowText
        Font.Height = -12
        Font.Name = 'Segoe UI'
        Font.Style = [fsBold]
        ParentBiDiMode = False
        ParentFont = False
        ReadOnly = True
        TabOrder = 8
      end
      object btPesquisarCliente: TBitBtn
        Left = 78
        Top = 47
        Width = 30
        Height = 25
        Hint = 'Consultar Clientes'
        Glyph.Data = {
          
          00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
        ParentShowHint = False
        ShowHint = True
        TabOrder = 1
        TabStop = False
        OnClick = btPesquisarClienteClick
      end
      object btCarregarPedidos: TBitBtn
        Left = 114
        Top = 47
        Width = 22
        Height = 24
        Hint = 'Consulta de Pedidos j'#225' realizados'
        Glyph.Data = {
          
          C6C6C6C6C6C6C6C6C6C6C6C6C6C6C6C6C6FF00FFFF00FFFF00FF}
        ParentShowHint = False
        ShowHint = True
        TabOrder = 2
        TabStop = False
        OnClick = btCarregarPedidosClick
      end
      object btExcluirPed: TBitBtn
        Left = 141
        Top = 47
        Width = 22
        Height = 24
        Hint = 'Cancelar Pedido j'#225' realizado'
        Glyph.Data = {
          
          00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
        ParentShowHint = False
        ShowHint = True
        TabOrder = 3
        TabStop = False
        OnClick = btExcluirPedClick
      end
    end
    object GradeProdutos: TDBGrid
      Left = 4
      Top = 201
      Width = 1036
      Height = 309
      Align = alClient
      DataSource = dsPedidoProduto
      Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit, dgTitleClick, dgTitleHotTrack]
      TabOrder = 2
      TitleFont.Charset = DEFAULT_CHARSET
      TitleFont.Color = clWindowText
      TitleFont.Height = -12
      TitleFont.Name = 'Segoe UI'
      TitleFont.Style = []
      OnDblClick = GradeProdutosDblClick
      OnKeyDown = GradeProdutosKeyDown
      Columns = <
        item
          Alignment = taCenter
          Expanded = False
          FieldName = 'PEDIDO_CODIGOITEM'
          ReadOnly = True
          Title.Alignment = taCenter
          Title.Caption = 'C'#243'digo Item'
          Visible = True
        end
        item
          Alignment = taCenter
          Expanded = False
          FieldName = 'PEDIDO_NUMERO'
          ReadOnly = True
          Title.Alignment = taCenter
          Title.Caption = 'N'#250'mero Pedido'
          Visible = True
        end
        item
          Alignment = taCenter
          Expanded = False
          FieldName = 'PRODUTO_CODIGO'
          ReadOnly = True
          Title.Alignment = taCenter
          Title.Caption = 'C'#243'd. Produto'
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'PRODUTO_DESCRICAO'
          ReadOnly = True
          Title.Alignment = taCenter
          Title.Caption = 'Descri'#231#227'o'
          Width = 372
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'PEDIDOPRODUTO_QUANT'
          ReadOnly = True
          Title.Alignment = taCenter
          Title.Caption = 'Quantidade'
          Width = 78
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'PEDIDOPRODUTO_VLRUNIT'
          ReadOnly = True
          Title.Alignment = taCenter
          Title.Caption = 'Valor Unit.'
          Width = 74
          Visible = True
        end
        item
          Expanded = False
          FieldName = 'PEDIDOPRODUTO_VLRTOTAL'
          ReadOnly = True
          Title.Alignment = taCenter
          Title.Caption = 'Valor Total Prod.'
          Width = 96
          Visible = True
        end>
    end
    object pnlProduto: TPanel
      Left = 4
      Top = 105
      Width = 1036
      Height = 96
      Align = alTop
      TabOrder = 1
      ExplicitWidth = 1032
      object Label5: TLabel
        Left = 24
        Top = 26
        Width = 68
        Height = 15
        Caption = 'C'#243'd Produto'
      end
      object Label6: TLabel
        Left = 176
        Top = 26
        Width = 79
        Height = 15
        Caption = 'Nome Produto'
      end
      object Label7: TLabel
        Left = 24
        Top = 6
        Width = 48
        Height = 15
        Caption = 'Produtos'
      end
      object Label8: TLabel
        Left = 567
        Top = 26
        Width = 54
        Height = 15
        Caption = 'Valor Unit.'
      end
      object Label9: TLabel
        Left = 480
        Top = 26
        Width = 62
        Height = 15
        Caption = 'Quantidade'
      end
      object Label13: TLabel
        Left = 655
        Top = 26
        Width = 71
        Height = 15
        Caption = 'Total Produto'
      end
      object edQuantidade: TEdit
        Left = 480
        Top = 47
        Width = 62
        Height = 23
        MaxLength = 6
        TabOrder = 3
        OnExit = edQuantidadeExit
        OnKeyPress = edQuantidadeKeyPress
      end
      object edTotalProduto: TEdit
        Left = 655
        Top = 47
        Width = 71
        Height = 23
        TabStop = False
        MaxLength = 16
        ReadOnly = True
        TabOrder = 5
      end
      object edCodProd: TEdit
        Left = 24
        Top = 47
        Width = 48
        Height = 23
        Hint = 'Digite o C'#243'digo do Produto ou F2 para Consultar'
        NumbersOnly = True
        ParentShowHint = False
        ShowHint = True
        TabOrder = 0
        OnExit = edCodProdExit
        OnKeyDown = edCodProdKeyDown
      end
      object edPrecoVenda: TEdit
        Left = 567
        Top = 47
        Width = 65
        Height = 23
        MaxLength = 6
        TabOrder = 4
        OnExit = edPrecoVendaExit
        OnKeyPress = edPrecoVendaKeyPress
      end
      object edNomeProduto: TEdit
        Left = 176
        Top = 47
        Width = 286
        Height = 23
        TabStop = False
        ReadOnly = True
        TabOrder = 2
      end
      object btIncluir: TBitBtn
        Left = 741
        Top = 47
        Width = 44
        Height = 23
        Hint = 'Incluir Produto'
        Glyph.Data = {
          
          D97851D46F55CE7371C986AFC6B3FF00FFFF00FFFF00FFFF00FF}
        ParentShowHint = False
        ShowHint = True
        TabOrder = 6
        OnClick = btIncluirClick
      end
      object btExcluir: TBitBtn
        Left = 800
        Top = 46
        Width = 44
        Height = 25
        Hint = 'Excluir Produto do Pedido'
        Glyph.Data = {
          
          00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
        ParentShowHint = False
        ShowHint = True
        TabOrder = 7
        OnClick = btExcluirClick
      end
      object btPesquisarProduto: TBitBtn
        Left = 78
        Top = 46
        Width = 30
        Height = 25
        Hint = 'Consultar Produtos'
        Glyph.Data = {
          
          00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
        ParentShowHint = False
        ShowHint = True
        TabOrder = 1
        TabStop = False
        OnClick = btPesquisarProdutoClick
      end
    end
    object Panel4: TPanel
      Left = 4
      Top = 510
      Width = 1036
      Height = 45
      Align = alBottom
      TabOrder = 3
      ExplicitTop = 509
      ExplicitWidth = 1032
      object Label11: TLabel
        Left = 781
        Top = 2
        Width = 94
        Height = 15
        Caption = 'Valor Total Pedido'
        FocusControl = DBEdit2
      end
      object DBEdit2: TDBEdit
        Left = 781
        Top = 18
        Width = 113
        Height = 23
        DataField = 'PEDIDO_VALORTOTAL'
        DataSource = dsPedidoCabecalho
        ReadOnly = True
        TabOrder = 0
      end
    end
  end
  object sqlCliente: TFDQuery
    Connection = DM.ConexaoDB
    SQL.Strings = (
      'SELECT '
      'CLIENTE_CODIGO,'
      'CLIENTE_NOME,'
      'CLIENTE_CIDADE,'
      'CLIENTE_UF'
      'FROM CLIENTE'
      'WHERE CLIENTE_CODIGO = :CLIENTE_CODIGO')
    Left = 964
    Top = 169
    ParamData = <
      item
        Name = 'CLIENTE_CODIGO'
        DataType = ftInteger
        ParamType = ptInput
        Value = Null
      end>
    object sqlClienteCLIENTE_CODIGO: TFDAutoIncField
      FieldName = 'CLIENTE_CODIGO'
      Origin = 'CLIENTE_CODIGO'
      ProviderFlags = [pfInWhere, pfInKey]
      ReadOnly = True
    end
    object sqlClienteCLIENTE_NOME: TStringField
      AutoGenerateValue = arDefault
      FieldName = 'CLIENTE_NOME'
      Origin = 'CLIENTE_NOME'
      Size = 60
    end
    object sqlClienteCLIENTE_CIDADE: TStringField
      AutoGenerateValue = arDefault
      FieldName = 'CLIENTE_CIDADE'
      Origin = 'CLIENTE_CIDADE'
      Size = 80
    end
    object sqlClienteCLIENTE_UF: TStringField
      AutoGenerateValue = arDefault
      FieldName = 'CLIENTE_UF'
      Origin = 'CLIENTE_UF'
      FixedChar = True
      Size = 2
    end
  end
  object dsCliente: TDataSource
    DataSet = sqlCliente
    Left = 996
    Top = 169
  end
  object sqlProduto: TFDQuery
    Connection = DM.ConexaoDB
    SQL.Strings = (
      'SELECT'
      'PRODUTO_CODIGO,'
      'PRODUTO_DESCRICAO,'
      'PRODUTO_PRECOVENDA'
      'FROM PRODUTO'
      'WHERE PRODUTO_CODIGO = :PRODUTO_CODIGO')
    Left = 912
    Top = 112
    ParamData = <
      item
        Name = 'PRODUTO_CODIGO'
        DataType = ftInteger
        ParamType = ptInput
        Value = Null
      end>
    object sqlProdutoPRODUTO_CODIGO: TFDAutoIncField
      FieldName = 'PRODUTO_CODIGO'
      Origin = 'PRODUTO_CODIGO'
      ProviderFlags = [pfInWhere, pfInKey]
      ReadOnly = True
    end
    object sqlProdutoPRODUTO_DESCRICAO: TStringField
      AutoGenerateValue = arDefault
      FieldName = 'PRODUTO_DESCRICAO'
      Origin = 'PRODUTO_DESCRICAO'
      Size = 80
    end
    object sqlProdutoPRODUTO_PRECOVENDA: TBCDField
      AutoGenerateValue = arDefault
      FieldName = 'PRODUTO_PRECOVENDA'
      Origin = 'PRODUTO_PRECOVENDA'
      DisplayFormat = '0.00'
      currency = True
      Precision = 6
      Size = 2
    end
  end
  object dsProduto: TDataSource
    DataSet = sqlProduto
    Left = 976
    Top = 112
  end
  object dsPedidoProduto: TDataSource
    DataSet = cdsPedidoProduto
    Left = 980
    Top = 353
  end
  object sqlPedidoProduto: TFDQuery
    MasterSource = dsLigacao
    MasterFields = 'PEDIDO_NUMERO'
    DetailFields = 'PEDIDO_NUMERO'
    Connection = DM.ConexaoDB
    FetchOptions.AssignedValues = [evCache]
    FetchOptions.Cache = [fiBlobs, fiMeta]
    SQL.Strings = (
      'SELECT'
      'PEDIDO_CODIGOITEM,'
      'PEDIDO_NUMERO,'
      'PRODUTO_CODIGO,'
      #39#39' as PRODUTO_DESCRICAO,'
      'PEDIDOPRODUTO_QUANT,'
      'PEDIDOPRODUTO_VLRUNIT,'
      'PEDIDOPRODUTO_VLRTOTAL'
      'FROM PEDIDOS_PRODUTOS'
      'WHERE PEDIDO_NUMERO = :PEDIDO_NUMERO'
      'ORDER BY 1')
    Left = 904
    Top = 352
    ParamData = <
      item
        Name = 'PEDIDO_NUMERO'
        DataType = ftInteger
        ParamType = ptInput
        Value = Null
      end>
    object sqlPedidoProdutoPEDIDO_CODIGOITEM: TFDAutoIncField
      FieldName = 'PEDIDO_CODIGOITEM'
      Origin = 'PEDIDO_CODIGOITEM'
      ProviderFlags = [pfInWhere, pfInKey]
      ReadOnly = True
    end
    object sqlPedidoProdutoPEDIDO_NUMERO: TIntegerField
      FieldName = 'PEDIDO_NUMERO'
      Origin = 'PEDIDO_NUMERO'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
    end
    object sqlPedidoProdutoPRODUTO_CODIGO: TIntegerField
      FieldName = 'PRODUTO_CODIGO'
      Origin = 'PRODUTO_CODIGO'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
    end
    object sqlPedidoProdutoPEDIDOPRODUTO_VLRUNIT: TBCDField
      FieldName = 'PEDIDOPRODUTO_VLRUNIT'
      Origin = 'PEDIDOPRODUTO_VLRUNIT'
      DisplayFormat = '0.00'
      currency = True
      Precision = 6
      Size = 2
    end
    object sqlPedidoProdutoPEDIDOPRODUTO_VLRTOTAL: TBCDField
      FieldName = 'PEDIDOPRODUTO_VLRTOTAL'
      Origin = 'PEDIDOPRODUTO_VLRTOTAL'
      DisplayFormat = '0.00'
      currency = True
      Precision = 14
      Size = 2
    end
    object sqlPedidoProdutoPEDIDOPRODUTO_QUANT: TBCDField
      FieldName = 'PEDIDOPRODUTO_QUANT'
      Origin = 'PEDIDOPRODUTO_QUANT'
      Required = True
      Precision = 6
      Size = 2
    end
    object sqlPedidoProdutoPRODUTO_DESCRICAO: TStringField
      AutoGenerateValue = arDefault
      DisplayWidth = 80
      FieldName = 'PRODUTO_DESCRICAO'
      Origin = 'PRODUTO_DESCRICAO'
      ProviderFlags = []
      Size = 80
    end
  end
  object dsPedidoCabecalho: TDataSource
    DataSet = cdsCabecalho
    OnDataChange = dsPedidoCabecalhoDataChange
    Left = 976
    Top = 280
  end
  object cdsPedidoProduto: TClientDataSet
    Aggregates = <>
    DataSetField = cdsCabecalhosqlPedidoProduto
    Params = <>
    Left = 952
    Top = 352
    object cdsPedidoProdutoPEDIDO_CODIGOITEM: TIntegerField
      FieldName = 'PEDIDO_CODIGOITEM'
    end
    object cdsPedidoProdutoPEDIDO_NUMERO: TIntegerField
      FieldName = 'PEDIDO_NUMERO'
      Origin = 'PEDIDO_NUMERO'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
    end
    object cdsPedidoProdutoPRODUTO_CODIGO: TIntegerField
      FieldName = 'PRODUTO_CODIGO'
      Origin = 'PRODUTO_CODIGO'
      ProviderFlags = [pfInUpdate, pfInWhere, pfInKey]
    end
    object cdsPedidoProdutoPEDIDOPRODUTO_VLRUNIT: TBCDField
      FieldName = 'PEDIDOPRODUTO_VLRUNIT'
      Origin = 'PEDIDOPRODUTO_VLRUNIT'
      DisplayFormat = ',0.00'
      currency = True
      Precision = 6
      Size = 2
    end
    object cdsPedidoProdutoPEDIDOPRODUTO_VLRTOTAL: TBCDField
      FieldName = 'PEDIDOPRODUTO_VLRTOTAL'
      Origin = 'PEDIDOPRODUTO_VLRTOTAL'
      DisplayFormat = ',0.00'
      currency = True
      Precision = 15
      Size = 2
    end
    object cdsPedidoProdutoPEDIDOPRODUTO_QUANT: TBCDField
      FieldName = 'PEDIDOPRODUTO_QUANT'
      Required = True
      DisplayFormat = ',0.00'
      Precision = 6
      Size = 2
    end
    object cdsPedidoProdutoPRODUTO_DESCRICAO: TStringField
      FieldName = 'PRODUTO_DESCRICAO'
      Size = 80
    end
  end
  object dsLigacao: TDataSource
    DataSet = DM.sqlPedido
    Left = 840
    Top = 312
  end
  object dspCabecalho: TDataSetProvider
    DataSet = DM.sqlPedido
    Left = 912
    Top = 280
  end
  object cdsCabecalho: TClientDataSet
    Aggregates = <>
    Params = <
      item
        DataType = ftInteger
        Name = 'PEDIDO_NUMERO'
        ParamType = ptInput
      end>
    ProviderName = 'dspCabecalho'
    Left = 944
    Top = 280
    object cdsCabecalhoPEDIDO_NUMERO: TIntegerField
      FieldName = 'PEDIDO_NUMERO'
    end
    object cdsCabecalhoCLIENTE_CODIGO: TIntegerField
      FieldName = 'CLIENTE_CODIGO'
    end
    object cdsCabecalhoPEDIDO_DATAEMISSAO: TDateField
      FieldName = 'PEDIDO_DATAEMISSAO'
    end
    object cdsCabecalhoPEDIDO_VALORTOTAL: TBCDField
      FieldName = 'PEDIDO_VALORTOTAL'
      DisplayFormat = '0.00'
      currency = True
      Precision = 14
      Size = 2
    end
    object cdsCabecalhosqlPedidoProduto: TDataSetField
      FieldName = 'sqlPedidoProduto'
    end
  end
end
